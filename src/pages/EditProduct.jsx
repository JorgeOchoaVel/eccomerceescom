...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setTitle(product.prodTitle);
        setPrice(product.prodPrice);
        setDescription(product.prodDescription);
        setCategId(product.prodCategId);
        setStock(product.prodStock);
        setPath(product.prodPath);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if ([title, price, description, categId, stock].includes("") || (!image && !path)) {
      setErrorInput(true);
    ...
    if (image) {
      ...
      newPath = `https://ecommerceescom-images.s3.amazonaws.com/${imageName}`;
    ...
    <div className="mt-[140px] sm:mt-20 max-w-screen-md mx-auto p-5">
      ...
      <div className="text-center mb-10">
        <h3 className="text-3xl sm:text-4xl leading-normal font-extrabold tracking-tight text-gray-900">
          Editar <span className="text-indigo-600">Producto</span>
        </h3>
      </div>
